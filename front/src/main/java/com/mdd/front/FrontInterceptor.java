...
public class FrontInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(@NonNull HttpServletRequest request,
        ...
        FrontThreadLocal.put("token", token);
        // 登录权限校验
        try {
            // 记录当前平台
            ...
            if (StringUtils.isEmpty(terminal)) {
                ...
                if (StringUtils.isEmpty(token)) {
                    FrontThreadLocal.put("terminal", ClientEnum.PC.getCode());
                } else {
                    ...
                    FrontThreadLocal.put("terminal", StringUtils.isNull(userSession) ? ClientEnum.H5.getCode() : userSession.getTerminal());
                }
            } else {
                FrontThreadLocal.put("terminal", terminal);
            }
            ...
            if (StringUtils.isNotNull(method) && method.isAnnotationPresent(NotLogin.class)) {
                try {
                    ...
                    if (userId > 0) {
                        ...
                        FrontThreadLocal.put("userId", uid);
                    }
...
//            }
            ...
            // 写入线程
            FrontThreadLocal.put("userId", user.getId());
            FrontThreadLocal.put("userSn", user.getSn());
            FrontThreadLocal.put("username", user.getAccount());
            FrontThreadLocal.put("nickname", user.getNickname());
            FrontThreadLocal.put("mobile", user.getMobile());
        }
    }
