...
public class WxMnpDriver {
    ...
    public static String getAccessTokens() throws Exception {
        Map<String, String> config = ConfigUtils.get("oa_setting");
        String appId = config.getOrDefault("app_id", "").trim();
        String appSecret = config.getOrDefault("app_secret", "").trim();
        String url = UrlUtils.getWeixinApiUrl() + "/cgi-bin/stable_token";
        JSONObject param = new JSONObject();
        param.put("grant_type", "client_credential");
        param.put("appid", appId);
        param.put("secret", appSecret);
        param.put("force_refresh", false);
        String result = HttpUtils.sendPost(url, JSONObject.toJSONString(param));;
    ...
    public static String getJsSdkGetTicket(String accessToken) {
        String jsSdkGetTicketUrl = UrlUtils.getWeixinApiUrl() + "/cgi-bin/ticket/getticket?access_token=%s&type=jsapi";
